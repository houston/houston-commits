...
class GitAdapterTest < ActiveSupport::TestCase
  ...
  test "#git_dir should return path when the repo is bare" do
    repo = GitAdapter.build(Project.new, bare_repo_path)
    assert_equal bare_repo_path, repo.send(:git_dir)
  end

  test "#git_dir should return the .git subdirectory when the repo is not bare" do
    path = File.expand_path("#{self.path}/../")
    repo = GitAdapter.build(Project.new, path)
    assert_equal "#{path}/.git", repo.send(:git_dir)
  end
...
STR
  ...
  test "should return a NullRepo if you give it an invalid path" do
    repo = GitAdapter.build Project.new, "nope"
  ...
  context "clone!" do
    ...
    should "work with a remote repo using the Git transport" do
      GitAdapter::RemoteRepo.new(temporary_path, github_repo).clone!
      assert File.exists?(fetch_head)
    end
  end
...
private
  ...
  def new_local_repo
    GitAdapter.connect bare_repo_path, bare_repo_path
  end

  def new_remote_repo
    GitAdapter.connect github_repo, temporary_path
  end
  ...
  def temporary_path
    @temporary_path ||= File.expand_path("#{path}/../tmp/#{Time.now.strftime("%Y%m%dT%H%M%S")}")
  end

  def fetch_head
    @fetch_head ||= "#{temporary_path}/HEAD"
  end
