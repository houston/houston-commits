...
class ProjectTest < ActiveSupport::TestCase
  ...
  context "A project's bare repo," do
    setup do
      system "cp -r #{bare_repo_path} #{path}/../tmp/test-01.git"
      @project = Project.create!(
        name: "Test",
        slug: "test-01",
        props: { "adapter.versionControl" => "Git", "git.location" => "git@github.com:houston/fixture.git"})
    end

    teardown do
      system "rm -rf #{path}/../tmp/test-*.git"
    end

    context "when the project's slug is changed," do
      should "be moved" do
        project.update_attributes slug: "test-02"
        test01_exists = File.exists?("#{path}/../tmp/test-01.git")
        test02_exists = File.exists?("#{path}/../tmp/test-02.git")
